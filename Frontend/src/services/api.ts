...
class ApiService {
  ...
  async subscribeWholesaler(
    localSellerId: number,
    wholesalerId: number,
  ): Promise<string> {
    return this.request<string>(
      `/local-seller/${localSellerId}/subscribe/${wholesalerId}`,
      { method: "POST" },
    );
  }

  async unsubscribeWholesaler(
    localSellerId: number,
    wholesalerId: number,
  ): Promise<string> {
    return this.request<string>(
      `/local-seller/${localSellerId}/unsubscribe/${wholesalerId}`,
      { method: "DELETE" },
    );
  }
