...
export function LocalSellerProfilePage() {
  ...
  const [editData, setEditData] = useState<UpdateSellerDTO>({});
  ...
  const handleUpdate = async () => {
    try {
      const updated = await sellerService.updateProfile(editData);
      setProfile(updated);
      setIsEditing(false);
    } catch (err: unknown) {
      if (err instanceof Error) setError(err.message);
        ...
        <div className="p-6 grid gap-6 md:grid-cols-2">
          ...
          <div className="space-y-4">
            ...
            <DetailItem
              icon={Store}
              label="ShopName"
              value={isEditing ? editData.shopName : profile.shopName}
              editable={isEditing}
              onChange={(val) => setEditData({ ...editData, shopName: val })}
            />
          </div>
        </div>
      </div>
...
function DetailItem({ icon: Icon, label, value, editable, onChange }: any) {
  return (
